...
assert.equal((await call(base+'/profile','GET',{}, {...user,organization_id:other})).profile.full_name,'');
assert.equal((await call(base+'/profile','PATCH',{full_name:'En otra empresa'},{...user,organization_id:other})).status,200);
assert.equal((await call(base+'/profile')).profile.full_name,'Mi nombre');
...
assert.equal((await call(base+'/profile')).profile.full_name,'Equipo inicial');
...
assert.equal((await call(base+'/profile','GET',{}, {...user,organization_id:other})).profile.full_name,'En otra empresa');
await query('update agency_collaborators set full_name=$1 where id=$2',['Editado en Equipo',person.id]);
assert.equal((await call(base+'/profile')).profile.full_name,'Editado en Equipo');
...
assert.equal((await query('select photo_url from agency_collaborators where id=$1',[person.id])).rows[0].photo_url,identityPhoto);
...
assert.equal((await call(base+'/people')).people[0].full_name,'Editado en Equipo');
await pg.exec(await fs.readFile('migrations/20260910_profile_identity.sql','utf8'));
assert.equal((await call(base+'/profile')).profile.full_name,'Editado en Equipo');
...
await pg.close();console.log('PASS: self-profile, per-company isolation, role protection, logos/colors, comments, duplicates, optimistic atomic batches, idempotent monthly templates, financial visibility');
