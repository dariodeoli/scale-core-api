...
begin
  for target in
    ...
    from pg_constraint
    where contype='c'
      and conrelid in ('invite_links'::regclass,'sessions'::regclass,'agency_role_permissions'::regclass)
      and pg_get_constraintdef(oid) like '%viewer%'
  loop
    execute format('alter table %s drop constraint %I', target.tbl, target.conname);
  end loop;
end $$;
alter table invite_links add constraint invite_links_role_check check (role in ('owner','admin','management','finance','sales','production','editor','viewer','collaborator'));
