...
await pg.exec(await (await import('node:fs/promises')).readFile(new URL('./migrations/20260912_platform_admin_bootstrap.sql',import.meta.url),'utf8'));
 ...
 await pg.exec(`
  ...
  alter table users add column deleted_at timestamptz,add column anonymized_at timestamptz,add column password_hash text,add column google_photo_url text,add column google_full_name text;
...
await pg.query("insert into agency_inventory_verifications(organization_id,inventory_id,verified_by_user_id,result,before_state,after_state) values(60,$1,11,'confirmed','{}'::jsonb,'{}'::jsonb)",[inventoryId]);
 ...
 await pg.query("insert into user_personal_identities(user_id,full_name,photo_url) values(11,'Tester Real','https://example.invalid/tester.png')");
 await pg.query("insert into agency_user_profiles(user_id,organization_id,full_name,photo_url) values(11,60,'Tester Real','https://example.invalid/tester.png')");
...
assert.equal((await pg.query('select count(*)::int as n from agency_inventory_verifications where verified_by_user_id=11')).rows[0].n,1,'inventory history survives the user deletion');
 ...
 assert.deepEqual((await pg.query("select full_name,photo_url from user_personal_identities where user_id=11")).rows[0],{full_name:'Deleted account',photo_url:null},'the personal identity is anonymized with the deleted account');
 assert.deepEqual((await pg.query("select full_name,photo_url from agency_user_profiles where user_id=11")).rows[0],{full_name:'Deleted account',photo_url:null},'the per-organization profile is anonymized with the deleted account');
