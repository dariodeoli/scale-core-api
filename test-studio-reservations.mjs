...
const users=[];
for(const [index,role] of ['owner','production','viewer','management'].entries()){
...
const [owner,producer,viewer,manager]=users;
...
const studioContext=await call('studio-context','GET',{},producer);assert.equal(studioContext.can_reserve,true);assert(studioContext.projects.some(row=>String(row.id)===String(project)));
...
assert.equal((await call('studio-reservations','POST',{...payload,responsible_user_ids:[owner.id]},producer)).status,400);
assert.equal((await call('studio-reservations','POST',{...payload,project_id:otherProject},producer)).status,400);
const reservation=(await call('studio-reservations','POST',payload,producer)).reservation;
...
assert.equal((await call('studio-reservations','POST',{...payload,title:'Overlap'},producer)).status,409);
const second=(await call('studio-reservations','POST',{...payload,title:'Ads de tarde',production_type:'ads',starts_at:end,ends_at:adjacentEnd},producer)).reservation;
...
const competing=await Promise.all([call('studio-reservations','POST',concurrentPayload,producer),call('studio-reservations','POST',concurrentPayload,producer)]);
assert.deepEqual(competing.map(result=>result.status).sort(),[201,409],'only one overlapping booking may commit');
assert.equal((await call(`studio-reservations/${reservation.id}`,'PATCH',{...payload,expected_version:99},producer)).status,409);
...
assert.equal((await call(`studio-reservations/${reservation.id}/cancel`,'POST',{expected_version:reservation.version},producer)).reservation.status,'cancelled');
assert.equal((await call('studio-reservations','POST',{...payload,title:'Nueva toma'},producer)).status,201,'cancelled slots are available again');
...
assert.equal((await call('studio-reservations','POST',{...payload,space_id:space.id,title:'No new inactive space'},producer)).status,400);
