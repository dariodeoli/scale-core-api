...
export async function financialForecast({req,res,url,db,session,send}) {
 ...
 try {
  ...
  const contractedClients=(await db.query(`
   with bounds as (select $2::date as start_on,($2::date+interval '1 month')::date as end_on),
   terms as (
    select t.client_id,t.currency,t.invoice_required,
   ...
   select t.client_id::text as client_id,c.name as client_name,t.currency,t.contracted_amount,coalesce(i.invoiced_amount,0) as invoiced_amount,t.invoice_required,
